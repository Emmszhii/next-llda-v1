"use client";
import { setError } from "@/src/store/StoreAction";
import { useStore } from "@/src/store/StoreContext";
import React from "react";

const MessageErrorModal = () => {
  ...
  const isServerErrorMessage =
    !store.message || store.message?.toLowerCase()?.includes("empty records");
  ...
  return (
    <>
      <div
        className="bg-red-200 px-4 py-3 mt-4 rounded-sm flex items-center justify-between gap-1 text-xs"
        ref={ref}
      >
